...
const ProgressCircle: React.FC<{ progress: number; status: 'safe' | 'warning' | 'danger'; strokeWidth?: number }> = ({
  progress,
  status,
  strokeWidth = 10,
}) => {
  ...
  const colorClasses = {
    safe: 'stroke-green-500 dark:stroke-green-400',
    warning: 'stroke-amber-500 dark:stroke-amber-400',
    danger: 'stroke-red-500 dark:stroke-red-400',
  };

  const trackColorClasses = {
      safe: 'stroke-green-200 dark:stroke-green-500/30',
      warning: 'stroke-amber-200 dark:stroke-amber-500/30',
      danger: 'stroke-red-200 dark:stroke-red-500/30'
  }

  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${viewBoxSize} ${viewBoxSize}`} className="transform -rotate-90">
      <circle
        className={trackColorClasses[status]}
      ...
      />
      <circle
        className={`${colorClasses[status]} transition-[stroke-dashoffset] duration-500 ease-out`}
        strokeWidth={strokeWidth}
...
export const CalculatorDisplay: React.FC<CourseCardProps> = React.memo(({
  ...
  const calculation = useMemo(() => {
    ...
    const defaultFirstWarning = isThreeCredit ? 4 : 2;
    const defaultSecondWarning = isThreeCredit ? 2 : 1;
  ...
  const header = (
    <div className="grid grid-cols-[auto_1fr_auto] items-center gap-3">
        ...
        <div className="flex flex-col self-stretch justify-center no-print">
            ...
            <button onClick={() => onMove(id, 'down')} disabled={isLast} className="p-1 rounded-full text-slate-400 dark:text-slate-500 disabled:opacity-30 disabled:cursor-not-allowed hover:bg-slate-100 dark:hover:bg-slate-700"> <ArrowDownIcon /> </button>
        </div>
        ...
        <div className="flex items-center space-x-0 sm:space-x-1 flex-shrink-0 no-print">
            <button onClick={() => openModal(setIsAbsenceCombosModalOpen)} className="hidden sm:flex items-center gap-1 text-sm text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 font-semibold p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                <CalculatorIcon className="h-4 w-4" /> <span>Scenarios</span>
            </button>
            <button onClick={() => openModal(setIsSettingsOpen)} aria-label={`Custom rules for ${course.name}`} className="p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700"><SettingsIcon /></button>
            <button onClick={() => onRemove(id)} aria-label={`Remove ${course.name}`} className="p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700"><TrashIcon /></button>
            <button onClick={() => setIsCollapsed(!isCollapsed)} aria-label="Toggle card visibility" className="p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700">
  ...
  if (isUncertainPolicy) {
    return (
      <div className={`bg-white dark:bg-slate-800 rounded-xl shadow-md p-4 sm:p-5 border border-slate-200 dark:border-slate-700 printable-card`}>
        {header}
        {!isCollapsed && (
    ...
    const combos = useMemo(() => {
        ...
        if (hasNoTutorial) {
            ...
            if (maxLectures !== Infinity && maxLectures >= 0) {
                 for (let l = 0; l <= maxLectures; l++) results.push({ lectures: l, tutorials: 0 });
            }
        ...
        for (let l = 0; (l * lectureCost) <= remainingPoints; l++) {
            ...
            if (t !== Infinity) results.push({ lectures: l, tutorials: t });
        }
        return results;
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 no-print" onClick={onClose}>
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-md p-6 relative" onClick={e => e.stopPropagation()}>
                <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:text-slate-400 dark:hover:text-slate-200"> <XIcon /> </button>
                ...
                <p className="text-slate-600 dark:text-slate-300 mb-4"> With your remaining <span className="font-bold text-indigo-600 dark:text-indigo-400">{calculation.remainingPoints} points</span>, you can still miss any of the following combinations: </p>
...
const CustomRulesModal: React.FC<{
    ...
    const isThreeCredit = course.creditHours === 3;
    const defaultRules = {
        totalPoints: isThreeCredit ? 10 : 6, lectureCost: isThreeCredit ? 2 : 1, tutorialCost: 1,
        firstWarning: isThreeCredit ? 4 : 2, secondWarning: isThreeCredit ? 2 : 1,
    };
    
    const [formState, setFormState] = useState(() => {
        const initialState = customRules || defaultRules;
        return {
            totalPoints: String(initialState.totalPoints), lectureCost: String(initialState.lectureCost),
            tutorialCost: String(initialState.tutorialCost), firstWarning: String(initialState.firstWarning),
    ...
    const handleSave = () => {
        onSave({
            totalPoints: parseInt(formState.totalPoints, 10) || 0, lectureCost: parseInt(formState.lectureCost, 10) || 0,
            tutorialCost: parseInt(formState.tutorialCost, 10) || 0, firstWarning: parseInt(formState.firstWarning, 10) || 0,
    ...
    const handleInputChange = (field: keyof CustomRules, value: string) => {
        if (/^\d*$/.test(value)) setFormState(prev => ({...prev, [field]: value }));
    };
